<template>
  <div
    class="shadow border-b border-gray-100 px-3 overflow-hidden relative z-10"
  >
      ...
      <div class="layout-tags-container whitespace-nowrap">
        ...
        >
          ...
          <el-icon
            ...
            class="text-xs hover:bg-gray-300 hover:text-white rounded-full leading-3 p-0.5 ml-1 -mr-1"
            @click="removeTag(v)"
            ><el-icon-close
          /></el-icon>
        </span>
  ...
  >
    ...
    <li class="px-4 py-2 cursor-pointer hover:bg-gray-200" @click="closeOther">
      关闭其它
    </li>
    <li
      class="px-4 py-2 cursor-pointer hover:bg-gray-200"
      @click="removeAllTagNav"
    >
      关闭所有
    </li>
  </ul>
...
const rightMenu = () => {
  ...
  const contextRightMenu = (v, event) => {
    ...
    nextTick(() => {
      ...
      if (
        event.clientX + rightMenuEl.value.offsetWidth >
  ...
  const refresh = () =>
    refreshViews(
      currentRightTags.path === route.path ? "replace" : "push",
      currentRightTags.path,
      currentRightTags.name
    );
  ...
  watch(
    ...
    () =>
      nextTick(() => {
        ...
        nextTick(() => {
          const itemWidth = layoutTagsItem.value
            .filter((v) => v)
            .reduce((acc, v) => {
              const val = v;
...
export default defineComponent({
  ...
  setup() {
    ...
    const route = useRoute();
    const removeTag = (v) => removeTagNav({ cPath: route.path, tagsList: v });

    onMounted(() => {
      addCachedViews({ name: route.name, noCache: route.meta.noCache });
    });
